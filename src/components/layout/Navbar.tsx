...
export const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[60] flex items-center justify-between px-6 py-5 transition-colors duration-300"
        style={{ 
          background: isOpen ? "rgba(16,15,13,1)" : "rgba(16,15,13,0.88)", 
          backdropFilter: "blur(16px)", 
          borderBottom: "1px solid rgba(221,216,207,0.06)" 
        }}>
        <span className="font-serif italic text-xl relative z-10" style={{ color: "#C1533B" }}>J. Ximénez</span>
        
        {/* Desktop Links */}
        <div className="hidden md:flex gap-8 text-xs tracking-widest uppercase relative z-10" style={{ color: "rgba(221,216,207,0.4)" }}>
          {links.map((link) => (
            <a key={link.href} href={link.href} className="hover:text-[#C1533B] transition-colors">{link.label}</a>
          ))}
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#100F0D]"
          >
